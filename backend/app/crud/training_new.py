...
def create_training(db: Session, training_data: TrainingCreate, user_id: int, session_id: Optional[str] = None) -> Optional[Union[Training, SessionTraining]]:
    ...
    # Проверяем существование пользователя (кроме гостевого user_id=0)
    if user_id != 0:
    ...
    # Преобразуем строку TrainingType в значение enum
    ...
    # Устанавливаем JSON поля (используем camelCase свойства)
    if training_data.poolTraining is not None:
        db_training.poolTraining = training_data.poolTraining
    if training_data.depthTraining is not None:
        db_training.depthTraining = training_data.depthTraining
    if training_data.gymTraining is not None:
        db_training.gymTraining = training_data.gymTraining
